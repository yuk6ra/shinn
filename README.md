...
## Overview
### Vision
次世代の信用インフラストラクチャー（Trust Data Infrastructure）によって新しい社会システムを実装する

### Mission
世の中のあらゆるクレデンシャル・データのコンポーザビリティ（Composability）とスケーラビリティ（Scalability）をなめらかにする

### Problem
既存のオンチェーン・クレデンシャル・データのプラットフォームは、マルチチェーンで展開される傾向があり、データという視点では相互運用性に欠けている。新興チェーンやその上に展開するDAppsのプロジェクトにとって拡張性ないサービス設計になっていることは、クレデンシャル・データを扱うプラットフォームにとっても、Web3プロジェクトやユーザにとっても、将来的に大きな課題になる可能性が高い。

### Solution
クレデンシャル・データ特化のL1パブリック・ブロックチェーン『SHINN Network』
...
## Problems
現在のオンチェーン・クレデンシャル・データのプラットフォームは『スケーラビリティがない』という課題がある。つまり、特定のブロックチェーン上のDApps間でコンポーザビリティは発揮されるが、クレデンシャル・データは単一のチェーンに依存してしまうため、将来を見据えた拡張性がない。そして、ユーザのアイデンティティを示すデータはチェーン間で自由に移動することができない。
...
| Title | Note |
| :----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|[AKINDO](https://app.akindo.io/communities/o6V33d6JecrJ91xB/products/kz8QQvrDmcR7d8AE)| Bootcamp2（提出用） |
